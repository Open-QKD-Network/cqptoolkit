...
In order for the data transfer to begin, the symetric key must first be exchanged, when more that two nodes are involved It was decided that key xor'ing would be used.  This allows multiple nodes in the chain without too much overhead.
...
Interfaces between instances, such as those between the site and the SDN will be defined with [gRPC](https://grpc.io/). The interfaces are defined using the [Protocol Buffer IDL](https://developers.google.com/protocol-buffers/) with gRPC extensions. They are built into a sepate QKDInterfaces library.

## Future work

* Keys should be manged and distroyed, the requester could also specify the use of the key to allow the desision to be made.
...
- The developmental libraries provided are not fit for purpose.
    ...
    - Built for an OS which is no longer suppored (Ubuntu 12.04)
...
### The IQSequence program
    ...
    @startuml Interfaces-Clavis2ConectionDiagram
        ...
        title Interfaces - Clavis 2 Conection Diagram
...
Be carful that any bridge you create doesn't interfeer with existing networks/subnets
...
```
The Docker file for cnfiguring the system

...
docker stop chromium-qkd 2>&1 2>/dev/null
...
sleep 1
byobu split-window -h ../external/chromium/run-chromium.sh --pskhsm=localhost:8001 http://localhost:8080 https://localhost:8443
